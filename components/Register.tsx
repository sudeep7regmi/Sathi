...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (activeTab === "PLAYER") {
        ...
        formData.append("phoneNumber", playerData.phoneNumber);
        formData.append("location", playerData.location);
        formData.append("age", String(Number(playerData.age) || 0));
        formData.append("preferredPosition", playerData.preferredPosition);
        formData.append("skillLevel", playerData.skillLevel);

        // Profile image
        if (playerImage) {
          formData.append("profileImage", playerImage);
        }
      } else {
        // Owner fields
        ...
        formData.append("fullName", ownerData.fullName);
        formData.append("phoneNumber", ownerData.phoneNumber);
        formData.append("futsalName", ownerData.futsalName);
        formData.append("futsalLocation", ownerData.futsalLocation);
      }
  ...
  return (
    <div className="bg-[#F0FDF4] min-h-screen flex flex-col items-center justify-center p-6 selection:bg-[#C8F55A] selection:text-black relative overflow-hidden py-16">
      ...
      <Link
        href="/"
        className="text-green-700 text-2xl tracking-tight mb-6 z-10"
        style={DISPLAY}
      >
      ...
      <div className="relative z-10 bg-white border border-gray-200 rounded-2xl p-8 w-full max-w-xl shadow-xl">
          ...
          {activeTab === "PLAYER" && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className={LABEL_CLASS}>Profile Image</label>
          ...
          {activeTab === "OWNER" && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={handleOwnerChange}
                    placeholder="business@futsal.com"
                    className={INPUT_CLASS}
                  />
                </div>
                <div>
              ...
              <div className="space-y-4 pt-5 border-t border-gray-200 mt-6">
                <h3
                  className="text-green-600 text-sm uppercase font-bold tracking-widest mb-3"
                  style={DISPLAY}
                >
                  Futsal Arena Details
                </h3>
                <div>
                  ...
                  <input
                    ...
                    name="futsalName"
                    required
                    value={ownerData.futsalName}
                    onChange={handleOwnerChange}
                    placeholder="e.g., Lalitpur Sports Arena"
                    className={INPUT_CLASS}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    name="futsalLocation"
                    required
                    value={ownerData.futsalLocation}
                    onChange={handleOwnerChange}
        ...
        <div className="mt-8 text-center text-sm">
          ...
          <Link
            href="/login"
            className="text-green-600 font-semibold hover:underline decoration-1 underline-offset-4 transition-colors"
          >
            Sign in here
          </Link>
        </div>
