...
export default async function Home() {
  ...
  return (
    <main className="bg-white text-[#111827] overflow-x-hidden min-h-screen selection:bg-[#C8F55A] selection:text-black">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map(({ icon, title, desc }) => (
              <div
                key={title}
                className="bg-white border border-gray-100 shadow-md hover:shadow-lg transition-shadow p-8 rounded-2xl"
              >
      ...
      <section
        id="venues"
        className="bg-[#0A1F1A] py-28 px-6 md:px-12 border-y border-white/5"
      >
        <div className="max-w-7xl mx-auto">
          <div className="max-w-3xl mb-16">
            ...
            <h2
              className="text-[#F0EDE6] leading-none text-4xl md:text-6xl tracking-tight"
              style={DISPLAY}
            >
              PREVIEW TOP COURTS
            </h2>
          ...
          {featuredGrounds.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {featuredGrounds.map((ground) => (
                <div
                  key={ground.id}
                  className="bg-[#12161A] border border-white/5 rounded-2xl overflow-hidden transition-all duration-300 hover:border-white/10 group"
                >
                  ...
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-2">
                      <h3
                        className="text-[#F0EDE6] text-xl font-bold tracking-wide line-clamp-1"
                        style={DISPLAY}
                      >
                        {ground.name}
                      </h3>
                    ...
                    <div className="flex items-center justify-between border-t border-white/5 pt-4">
                      <div className="flex flex-col">
                        ...
                        <span
                          className="text-[#C8F55A] text-xl font-black"
                          style={DISPLAY}
                        >
                      ...
                      <Link
                        href="/register"
                        className="bg-white/5 hover:bg-white/10 text-white text-xs px-4 py-2 rounded font-semibold transition-all"
                      >
                        Check Slots
                      </Link>
                    </div>
      ...
      <section className="py-32 px-6 text-center relative overflow-hidden bg-[#F0FDF4]">
        ...
        <div className="relative max-w-2xl mx-auto">
          <h2
            className="text-[#111827] leading-[0.9] mb-6 tracking-tighter"
          ...
          <Link
            href="/register"
            className="inline-block bg-[#C8F55A] text-[#111] px-10 py-4 rounded-xl text-lg font-bold tracking-wide uppercase hover:bg-[#A8D448] transition-all shadow-lg shadow-green-200"
            style={DISPLAY}
          >
