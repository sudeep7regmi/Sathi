...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#0B0C10] flex text-[#F0EDE6] selection:bg-[#C8F55A] selection:text-black relative overflow-hidden">
      ...
      {/* Admin Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-full md:w-72 bg-[#12161A] border-r border-white/5 flex flex-col transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} md:relative`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          {navLinks.map((link) => {
            ...
            return (
              <button 
                key={link.path} 
                onClick={() => { setIsSidebarOpen(false); router.push(link.path); }} 
                ...
                style={DISPLAY}
              >
                <IconComponent className={`w-5 h-5 ${isActive ? 'text-black' : 'text-white/50'}`} />
                <span>{link.name}</span>
              </button>
            );
          })}
